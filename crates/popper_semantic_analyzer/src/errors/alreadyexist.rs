...
impl Error for AlreadyExist {
    fn report(&self, color: ColorConfig, source: &str, file: &str) {
        ...
        report
            .with_code(23)
            .with_message(
                format!("Variable `{}` already exist at `{}`", self.recreated_at.0, self.created_at)
            )
            .with_label(
                ariadne::Label::new((file, self.recreated_at.1.into()))
                    .with_message(
                        format!("`{}` already exist", self.recreated_at.0.clone().fg(*variable))
                    )
            )
            .with_label(
                ariadne::Label::new((file, self.created_at.into()))
                    .with_message("created here")
