...
impl Compiler {
    ...
    pub fn declare(&mut self, declare: Declare) {
        ...
        for arg in declare.args.list {
            args.push(self.mir_ty_to_llvm_ty(arg));
        }
    ...
    pub fn compile_function(&mut self, func: popper_mir::mir_ast::Function) {
        ...
        for arg in func.args.args.clone() {
            args.push(self.mir_ty_to_llvm_ty(arg.ty));
        }
        ...
        self.env.insert(
            func.name.clone(),
            PopObject::new(fn_val.to_value(), function.clone().to_type_enum()),
        );
    ...
    pub fn compile_value(&mut self, value: MirValue) -> ValueEnum {
        return match value {
            MirValue::Const(cons) => match cons {
                Const::Int(int) => {
                    let ty = self.context.i64_type();
