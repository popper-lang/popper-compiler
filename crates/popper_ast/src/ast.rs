...
impl LangAst {
    ...
    fn dumps_node(&self, node: &LangNode) -> String {
        match &node.kind {
            ...
            LangNodeKind::Let(let_) => format!("Let {} = {}", self.dumps_symbol(let_.name.0), self.dumps_node(self.get(let_.value))),
            ...
            LangNodeKind::If { condition, then_branch, else_branch } => {
                let mut result = format!("If {} then {} ", self.dumps_node(self.get(*condition)), self.dumps_node(self.get(*then_branch)));
            ...
            LangNodeKind::FunctionCall { function, args } => {
                ...
                let args_str: Vec<String> = args.iter().map(|&id| self.dumps_node(self.get(id))).collect();
                format!("{}.call([{}])", func_str, args_str.join(", "))
            },
            LangNodeKind::Return(expr) => format!("Return({})", self.dumps_node(self.get(*expr))),
            LangNodeKind::FunctionDef { name, attrs, params, ret,  body, is_expr } => {
                let params_str: Vec<String> = params.iter().map(|param| format!("{}: {:?}", param.name.0.index(), param.ty)).collect();
                ...
                format!("FunctionDef({}) {}({}) -> {:?} {}", attr_str.join(","), self.dumps_symbol(name.0), params_str.join(", "), ret, s)
            },
        }
    }
    ...
    fn dumps_expr(&self, expr: &Expr) -> String {
        match expr {
            ...
            Expr::UnaryOp(op, node_id) => {
                ...
                format!("UnaryOp({}, {})", op_str, self.dumps_node(self.get(*node_id)))
            }
            Expr::Add(lhs, rhs) => format!("Add({}, {})", self.dumps_node(self.get(*lhs)), self.dumps_node(self.get(*rhs))),
            Expr::Sub(lhs, rhs) => format!("Sub({}, {})", self.dumps_node(self.get(*lhs)), self.dumps_node(self.get(*rhs))),
            Expr::Mul(lhs, rhs) => format!("Mul({}, {})", self.dumps_node(self.get(*lhs)), self.dumps_node(self.get(*rhs))),
            Expr::Div(lhs, rhs) => format!("Div({}, {})", self.dumps_node(self.get(*lhs)), self.dumps_node(self.get(*rhs))),
        }
    }
